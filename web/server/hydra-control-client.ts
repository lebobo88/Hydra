/**
 * web/server/hydra-control-client.ts
 *
 * Stdio MCP child client for the hydra_control MCP server.
 *
 * Forks the hydra-mem-client.ts idioms EXACTLY:
 ...
 *   - Handshake-gated single-flight via handshakeComplete + connectPromise
 *     (carries the C1 connect-race fix forward — no tools/call before handshake
 *     is complete; concurrent callers share one in-flight connectPromise).
 *
 ...
 *   python -m mcp_servers.hydra_control
 * with:
 ...
 *
 * WRITE surface:
 *   This client is the WRITE transport for the five gate-resume actions.
 *   It exposes only two tool calls:
 *     - ping()         → hydra.control.ping (liveness probe)
 *     - resume(...)    → hydra.workflow.resume (the only write tool)
 *   No read tools are accessible through this client — the read/write split
 *   is structural: reads go through HydraMemClient, writes through this client.
 */
...
// ---------------------------------------------------------------------------
// Hydra root discovery (mirrors hydra-mem-client.ts exactly)
// ---------------------------------------------------------------------------
...
function findHydraRoot(): string {
  ...
  // 2. Repo-relative: web/server/ → ../../ = Hydra root
  try {
    ...
    for (const c of candidates) {
      if (existsSync(join(c, 'mcp_servers', 'hydra_control', 'server.py'))) {
        return c;
      }
    }
  } catch {
...
// ---------------------------------------------------------------------------
// Resume result shape (mirrors hydra_control/server.py _launch_resume return)
// ---------------------------------------------------------------------------
...
export class HydraControlClient {
  ...
  // Single-flight serialization mutex (matches single-flight constraint on python side)
  private callChain: Promise<unknown> = Promise.resolve();

  // Handshake gate (C1 connect-race fix — carried forward verbatim)
  // -----------------------------------------------------------------------
  ...
  async connect(): Promise<void> {
    ...
    const p = spawn(pythonExe, ['-m', 'mcp_servers.hydra_control'], {
      cwd: root,
    ...
    if (!p.stdin || !p.stdout) {
      throw new Error('failed to attach stdio to hydra_control child process');
    }
    ...
    this.proc.on('exit', (code) => {
      ...
      const exitErr = new Error(
        `hydra_control child exited${code != null ? ` with code ${code}` : ''}`,
      );
  ...
  /**
   * Liveness probe — calls hydra.control.ping.
   * Returns the ping result or throws on failure.
   */
  async ping(): Promise<Record<string, unknown>> {
    return this.call<Record<string, unknown>>('hydra.control.ping', {});
  }

  /**
   * Typed resume helper — calls hydra.workflow.resume with the validated args.
   * The fixed cockpit envelope is injected server-side; browser never sets it.
   *
   * @param workflow_id - validated against WORKFLOW_ID_RE before calling
   * @param action      - one of the 5 resume actions
   * @param option      - optional argument (e.g. budget amount, squad list)
   */
  async resume(
    workflow_id: string,
    action: string,
    ...
    const args: Record<string, unknown> = { workflow_id, action };
  ...
  // --------------------------------------------------------------------------
  // Private helpers (mirror hydra-mem-client.ts exactly)
  // --------------------------------------------------------------------------
  ...
  private send(method: string, params: Record<string, unknown>): Promise<unknown> {
    if (!this.proc) return Promise.reject(new Error('hydra-control client not connected'));
    ...
    return new Promise((resolve, reject) => {
      this.pending.set(id, {
        ...
        timer: setTimeout(() => {
          ...
          reject(new Error(`hydra-control: timeout waiting for response to ${method} (id=${id})`));
        }, CALL_TIMEOUT_MS),
      });
  ...
  private _callRaw<T>(tool: string, args: Record<string, unknown>): Promise<T> {
    return this.sendWithTimeout('tools/call', {
      name: tool,
