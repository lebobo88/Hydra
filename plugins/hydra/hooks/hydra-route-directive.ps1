...
if ($isAction) {
    if ($ecosystemAvailable) {
        ...
        Write-Output "  Mode: ATTENDED (always, for interactive sessions) — drive engineering in-context with visible engineer/judge subagents (plan -> step -> submit_host_result). Engine stays authoritative (ledger/budget/judge). Detached launch is automation-only (HYDRA_ALLOW_DETACHED=1); fleet --repos is the one sanctioned detached use from a session."
