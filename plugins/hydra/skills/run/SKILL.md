...
4. **HITL**: when a launch/ingest surfaces a pending gate, STOP and render it — operator resumes with `/hydra:approve` or `/hydra:resume` (which call `hydra.workflow.resume`).
   - **The plan leg** (the default now — `HYDRA_PLAN_PHASE` ships on; skipped only when the operator set `HYDRA_PLAN_PHASE=0` or triage called the work `trivial`): the first thing the cursor hands back is a `planning` task, and every other task is held behind the plan barrier until it resolves. Drive it exactly like any other attended task — spawn the `host_action`'s named agent, then `hydra.workflow.submit_host_result` — and submit the authored `PLAN` in that result's `emitted_envelopes`. The engine validates it, writes `docs/plans/plan-<slug>.html`, judges it, and parks at `plan_gate`. **Stop there and render the plan**: its repo-relative path, the step table with dependencies, the judge verdict and vendor, the budget estimate, and the open-question count. The operator approves, rejects, or requests a revision with `--modify-plan`. Only on approval do the plan's steps become dispatchable tasks. Never author the plan yourself and never write the artifact by hand — the `plan-scribe` head and the ingest branch are the only writers.
   - A terminal of `blocked_on_failed_dependency` is **not** `ready_to_finalize`. It means a task's `depends_on` can never be satisfied because an upstream task surfaced or aborted. Render it as the distinct condition it is rather than finalizing over it.
5. **Synthesis**: when the attended loop reports `{status:"ready_to_finalize"}`, call `hydra.workflow.finalize {workflow_id}` — the engine materialises the attended results, runs `synthesis → judge_synthesis → postcheck` over them, persists the `DECISION_RECORD` to episodic memory and drives the phase terminal. Present the returned engine `DECISION_RECORD` (decision, rationale, dissents, `artifact_refs`) plus artifact/commit paths, alongside the workflow `trace.jsonl`. It is idempotent (`already_finalized`) and refuses with `tasks_pending` while any task is still open.
